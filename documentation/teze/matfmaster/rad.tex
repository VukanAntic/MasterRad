...
\posveta{Овај рад посвећујем свом оцу, мајци, брату, пријатељима и колегама. др. Владимиру Филиповићу се захваљујем
...
\item Izrada klijenta zasnovanog na MVC arhitekturi \cite{fowler-mvc} - U glavi \ref{ch:klijent} prikazano je kako je implementiran klijentski deo koda koji prati MVC arhitekturu i dodatne funkcionalnosti koje klijent podržava kao što je viđeta (eng. \textit{widget}).
...
Za IOS uređaje takođe je implementiran i viđet (eng. \textit{widget}) \cite{widgetkit} 
...
Kad je implementacija viđeta u pitanju, bitna je napomena da on \textit{nije deo aplikacije},
...
metode napisane u TypeScript jeziku. Komunikacija između glavne aplikacije i viđeta je preko deljenog prostora 
(engl. \textit{App Group}).


Aplikacija preuzme sliku koju treba prikazati korisniku tog dana, sačuvaje u zajednički prostor, zajedno 
sa informacijama ko je bio slikar, ime slike, i za koji datum je slika namenjena, i onda viđet može 
da prikaže najnoviju sliku korisniku.


\begin{lstlisting}[language=JavaScript,
  caption={Slanje informacije o najnovijoj slici viđetu},
  label={lst:widget}]
...
Sam viđet ima dva stanja. Kada je slika namenjena za trenutni dan, ona je prikazana. Kada nije,
...
\begin{figure}[htbp]
  ...
  \end{minipage}
  \caption{Dva stanja viđeta na početnom ekranu}
  \label{fig:widget}
\end{figure}
...
Ugovor koji postoji između klijenta i servera iako donosi prednosti, donosi sa sobom i par mana:
...
kao i problem N + 1.
Kod gRPC-a postoji stroži oblik ugovora, jer se iz ugovora gradi komunikacija između klijenta i servera.
...
Kao posledica iskustva dobijenog izrade ove aplikacije, nijedan od protokola komunikacije se nije pokazao 
