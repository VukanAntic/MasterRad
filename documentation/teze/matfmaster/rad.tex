...
na upotrebu arhitektonskog stila REST. Pored njega postoje i jezik GraphQL i radni okvir gRPC,
koji donose drugačije ustupke po pitanju fleksibilnosti, veličine poruka i strogosti ugovora
između dve strane. 
...
\posveta{Овај рад посвећујем свом оцу, мајци, брату, пријатељима и колегама на подршци у току израде овог рада. др. Владимиру Филиповићу се захваљујем на стрпљењу као и саветима у току израдe овог рада. Посебну захвалност дугујем партнеру Душици, на свој љубави пруженој у току студија.}
...
U prethodnim glava, sva komunikacija klijenta i servera bila je izvdojena u zasebne slojeve, (engl. \textit{Onion})
arhitektura na serveru gde spoljašnji sloj predstavlja komunikaciju sa klijentom, dok na klijentu interfejsi 
...
sastavne delove. Uz izvesno pojednostavljenje, ono se može izraziti kao
...
$t^{\mathit{obr}}_i$, odnosno na vreme obrade na serveru - što će se u analizi
rezultata pokazati kao značajno.
...
2000. godine u svojoj doktorskoj disertaciji \cite{fielding2000}. Iako je do sada spomenut kao protokol, 
bitna je napomena da REST nije protokol komunikacije, već \emph{arhitektonski stil}. On predstavlja skup pravila, 
koja dovode do sistema skalabilan, labavo spregnut i pogodan za keširanje. 
...
\begin{itemize}
  ...
  \item \textbf{Bezstanjnost} (eng. \textit{Statelessness}) - svaki zahtev sadrži sve podatke potrebne za
...
Značajan pojam REST-a jeste \textbf{resurs}. Ona predstavlja svaku celinu koja je od značaja za ceo sistem. 
U aplikaciji, to potrazumevaju umetničke slike, žanrovi ili autori. Svaki resurs ima svoju adresu, pri čemu se razne 
...
Glavni nedostaci REST-a jeste unapred definisanje odgovora servera. Upravo zbog toga, dolazi do sledećih situacija:
\begin{itemize}
  \item Prekomerno dohvatanje (eng. \textit{over-fetching}) - do ovog problema dolazi kada resurs čini više 
  polja nego što je klijentu potrebno. Iako su klijentu potrebne manje polja nego što resurs čini, on dobija dodatne nepotrebne informacije.
...
GraphQL predstavlja jezik upita koji unapred definiše koje sve podatke koje klijent može da zahteva od servera. Kreiran 
je od strane kompanije Fejsbuk (eng. \textit{Facebook}) 2012. godine za internu upotrebu, ali je postao 
otvorenog koda 2015. godine da može bilo ko da ga koristi. Osnovna zamisao protokola jeste da klijent opisuje oblik 
...
Ako klijentu nisu potrebni svi podaci unutar scheme, već samo podskup njenih atributa, ona može da zatraži upravo njih. 
...
\begin{lstlisting}[language=GraphQLSchema,
  caption={Deo sheme mikroservisa za slike},
  label={lst:gql-shema}]
type Artwork {
    id: Int!
    title: String!
...
populisan ili ne (da li atribut ne sme biti jednak eng. \textit{null}, definiše sa uzvičnikom).
...
\begin{itemize}
  \item Upiti (eng. \textbf{Query}) - operacija dohvatanja i mogu se izvršavati paralelno.
  \item Mutacije (eng. \textbf{Mutation}) - operacija koja podrazumeva promenu stanja koja će biti izvršena nad podacima.
  \item Pretplate (eng. \textbf{Subscription}) - dugotrajna komunikacija servera ka klijentu, gde server u bilo kom trenutku može da se obrati klijentu da postoje neki nov sadržaj.
\end{itemize}
...
pošto bi klijent mogao sve podatke potrebne za rad aplikacije da zataži od jednog poziva serveru. 
...
Primer koda \ref{lst:gql-mutation} predstavlja mutacije koje se vrše nad prefenrencama korisnika. Omogućava 
klijentu da uputi zahtev da se desi neka promena, kao što je promena omiljene slike uz pomoć njih.

\subsection{N + 1 problem}

Jedan od većih problematika GraphQL jeste n + 1 problem. On podrazumeva da ako klijent zahteva listu od n umetničkih slika, i 
onda za svaku od njih informaciju o slikaru,  
...
i onda ih izvršava kao jedan grupni upit.
...
Druga cena jeste potreba za ograničavanjem složenosti upita. Pošto klijent sam bira
upit koji će uputiti ka serveru, taj upit može biti duboko ugnježden koji server opterećuje. Iz tog razloga, 
sistemi uvode ograničavanja dubine.
...
Pored dodavanje scheme kojoj se definiše međusobna komunikacija, kao i mutacije i upite, potrebni 
...
2015. godine \cite{grpc-docs}. Za razliku od stila REST, koji je zasnovan na resursima i
...
Dve odluke čine osnovu ovog radnog okvira. Poruke se opisuju i kodiraju jezikom Protocol Buffers,
...
binarni format u koji se te poruke kodiraju \cite{protobuf}. Opis se piše u datoteci sa nastavkom
\texttt{.proto} i predstavlja ugovor kojeg su i klijent i server dužni da se pridržavaju. Iz tog 
dogovora se generiše kod na oba jezika, Java na serveru, i TypeScript na klijentu, pri čemu je onda moguća implementacija 
protokola. Deo opisa mikroservisa za slike moguće je videti u primeru
\ref{lst:grpc-proto}.
...
strane građene iz istog opisa.

Značajan pojam jeste \textbf{redni broj polja}. Svako polje unutar poruke nosi svoj broj, i
upravo se taj broj, a ne ime polja, upisuje u binarni zapis. On omogućava i razvoj šeme kroz
...
\begin{itemize}
  ...
  \item \textbf{Binarno uokvirivanje} (eng. \textit{Binary framing}) - poruke se prenose u
  binarnim okvirima umesto u tekstualnom obliku.
  \item \textbf{Sažimanje zaglavlja} (eng. \textit{Header compression}) - zaglavlja se ne
  prenose u celosti uz svaki zahtev, već se ponovljene vrednosti pamte.
...
\begin{itemize}
  ...
  poruka, a server odgovara tek nakon što taj niz bude završen.
...
Strogost ugovora, koja predstavlja glavnu prednost ovog radnog okvira, ujedno je i izvor njegovih
nedostataka:

\begin{itemize}
  \item \textbf{Nečitljivost poruka} - pošto je zapis binaran, tok komunikacije nije moguće
  pratiti bez posebnih alata, niti je zahtev moguće sastaviti ručno.
  \item \textbf{Korak generisanja koda} - svaka izmena opisa zahteva ponovno generisanje koda i
  na serveru i na klijentu. Za razliku od REST-a, gde je dodavanje polja izmena na jednom mestu,
  ovde je reč o izmeni koja obavezno dodiruje obe strane i sam postupak izgradnje projekta.
  \item \textbf{Gubitak keširanja na nivou protokola HTTP} - kao i kod GraphQL-a, posrednici u
  mreži ne mogu zaključiti da li je odgovor moguće ponovo upotrebiti.
  \item \textbf{Nedostatak podrške u okruženju veb pregledača} - pregledač ne dozvoljava
  neposredan pristup okvirima protokola HTTP/2, pa gRPC u tom okruženju nije moguće koristiti bez
  posrednika.
\end{itemize}
...
aplikacije za svakodnevni prikaz umetničkih dela, izgrađene u celosti u tu svrhu. Prikazan je
...
aplikacije ostali nepromenjeni. Svaki protokol predstavljen je osnovnom zamisli na kojoj počiva,
opisom svojih ograničenja i nedostataka, kao i prikazom implementacije na serveru i na klijentu.

Time što su sva tri protokola postavljena nad istom aplikacijom, njihove prednosti i mane bilo je
moguće uporediti neposredno. REST se pokazao kao najjednostavniji za uvođenje - dovoljna je
anotacija nad metodom, a sve ostalo obavlja radni okvir - i kao jedini kod kojeg keširanje na
nivou protokola HTTP ostaje upotrebljivo. Njegov nedostatak jeste odsustvo formalnog opisa
poruke, zbog čega se neslaganje između klijenta i servera otkriva tek u vreme izvršavanja, kao i
činjenica da oblik odgovora unapred određuje server, što vodi ka prekomernom i nedovoljnom
dohvatanju. GraphQL taj nedostatak uklanja time što oblik odgovora prepušta klijentu i time što
dozvoljava da se više logičkih upita objedini u jedan zahtev, ali to plaća gubitkom keširanja i
pomeranjem složenosti na serversku stranu, gde se javljaju problem N+1 i potreba za
ograničavanjem dubine upita. gRPC daje najstroži ugovor između dve strane, pošto se obe grade iz
istog opisa, pa greška u imenu metode ili u tipu argumenta ne može ni da se prevede, uz najmanje
poruke zahvaljujući binarnom zapisu. Zauzvrat traži korak generisanja koda pri svakoj izmeni
opisa, poruke čini nečitljivim.

Iz iskustva stečenog pri izradi ove aplikacije, nijedan od tri protokola ne nameće se kao najbolji
izbor u svim prilikama. REST ostaje razuman podrazumevani izbor, naročito kada interfejs koriste
klijenti nad kojima autor nema kontrolu, kada je čitljivost poruka bitna i kada se od keširanja
očekuje korist. GraphQL najviše donosi upravo u slučaju kakav je ovaj - jedan klijent sa
raznovrsnim ekranima, od kojih svakom treba drugačiji podskup istih podataka, i to iz više izvora
odjednom. gRPC se pokazao kao najprirodniji za komunikaciju između servisa unutar samog sistema,
gde su obe strane pod kontrolom istog autora i gde se od poruka ne očekuje da budu čitljive.
U izradi ovog rada, ulogu kominikacije između mikroservisa igrao je red poruka.

Neke od ideja za nastavak rada na ovoj temi bile bi merenje vremena odziva sva tri protokola pod
uslovima stvarne mobilne mreže, gde je vreme kružnog putovanja znatno veće nego u lokalnom
okruženju, pa se razlike među njima najjasnije ispoljavaju. Takođe, u GraphQL implementaciji bilo
bi moguće uvesti razrešavanje pojedinačnih polja uz grupisanje poziva obrascem \textit{DataLoader},
čime bi selekcija polja koju klijent vrši uticala i na obim posla nad bazom, a ne samo na veličinu
odgovora, kao i jedinstven ulaz koji objedinjuje šeme svih mikroservisa, čime bi bio rešen i
problem nedovoljnog dohvatanja.
